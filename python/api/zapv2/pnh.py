...
class pnh(object):
    ...
    def monitor(self, id, message, apikey=''):
        ...
        """
        return next(iter(list(self.zap._request(self.zap.base + 'pnh/action/monitor/', {'id' : id, 'message' : message, 'apikey' : apikey}).values())))

    def oracle(self, id, apikey=''):
        """
        This component is optional and therefore the API will only work if it is installed
        """
        return next(iter(list(self.zap._request(self.zap.base + 'pnh/action/oracle/', {'id' : id, 'apikey' : apikey}).values())))

    def start_monitoring(self, url, apikey=''):
        """
        This component is optional and therefore the API will only work if it is installed
        """
        return next(iter(list(self.zap._request(self.zap.base + 'pnh/action/startMonitoring/', {'url' : url, 'apikey' : apikey}).values())))

    def stop_monitoring(self, id, apikey=''):
        """
        This component is optional and therefore the API will only work if it is installed
        """
        return next(iter(list(self.zap._request(self.zap.base + 'pnh/action/stopMonitoring/', {'id' : id, 'apikey' : apikey}).values())))
